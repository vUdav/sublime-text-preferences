...
3. Переходим в папку пользовательских настроек вашей ОС:
    1. ```cd %AppData%\Sublime Text 3\Packages\ - для Windows```
    2. ```cd ~/.config/sublime-text-3/Packages/ - для Linux```
    3. ```cd ~/Library/Application\ Support/Sublime\ Text\ 3/Packages/ - для Mac```
